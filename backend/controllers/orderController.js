  ...
  try {
    ...
    if (items && items.length === 0) {
      ...
      const message = `
        ...
        <p>Thank you for shopping with Gadgets-Hub!</p>
      `;

      await sendEmail({
        email: req.user.email,
        subject: 'Gadgets-Hub - Order Confirmation',
        message
      });
...
export const getMyOrders = async (req, res) => {
  try {
    const orders = await Order.find({ userId: req.user._id });  //~ created during login
    res.json(orders); //~ for frontend
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
